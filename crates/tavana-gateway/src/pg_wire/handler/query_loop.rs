//! Main query processing loops
//!
//! Contains the main event loops that process client messages for both
...
use super::execution::{execute_query_streaming_with_scaler, execute_query_tls};
use super::extended::{
    handle_bind, handle_describe, handle_execute_empty, handle_execute_extended,
    handle_parse_extended,
};
use super::messages::{
    send_command_complete_generic, send_data_row_generic, send_error, send_error_generic,
    send_query_result_immediate, send_row_description_generic_with_formats,
    send_simple_result_generic,
};
...
use super::utils::{
    ...
    substitute_parameters, QueryExecutionResult, TRANSACTION_STATUS_ERROR,
        ...
        match msg_type[0] {
            ...
            b'Q' => {
                ...
                if query_trimmed.starts_with("DECLARE ") && query_trimmed.contains(" CURSOR ") {
                    debug!(query_trimmed = %query_trimmed, "Detected DECLARE CURSOR command (non-TLS)");
                    ...
                    Ok(query_token) => {
                        ...
                        let result = tokio::time::timeout(
                            ...
                            execute_query_streaming_with_scaler(
                                socket, worker_client, query_router, &query, user_id, smart_scaler,
                            )
                        ).await;
                ...
                if let Some(ref query) = prepared_query {
                    if !describe_sent_row_description {
                        ...
                        handle_execute_extended(
                            socket, &mut buf, worker_client, query_router, query, user_id,
                            smart_scaler, query_queue,
                        ).await?;
                    }
                } else {
...
pub(crate) async fn run_query_loop_generic<S>(
    socket: &mut S,
    worker_client: &WorkerClient,
    worker_client_pool: &WorkerClientPool,
    query_router: &QueryRouter,
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    loop {
        ...
        match msg_type[0] {
            ...
            b'Q' => {
                info!("Simple Query message received (TLS)");
                socket.read_exact(&mut buf).await?;
                let len = u32::from_be_bytes(buf) as usize - 4;
                let mut query_bytes = vec![0u8; len];
                socket.read_exact(&mut query_bytes).await?;
                ...
                info!("Received query (TLS): '{}' (len={}, bytes={:?})", 
                    &query[..query.len().min(100)], query.len(),
                ...
                if query_trimmed.contains("CURSOR") || query_trimmed.starts_with("DECLARE") || query_trimmed.starts_with("FETCH") {
                    info!(query_preview = %&query_trimmed[..query_trimmed.len().min(80)], "Processing potential cursor command");
                }
                
                if query_trimmed.starts_with("DECLARE ") && query_trimmed.contains(" CURSOR ") {
                    info!(query_trimmed = %query_trimmed, "Detected DECLARE CURSOR command");
                    let default_client = worker_client_pool.default_client();
                    if let Some(result) = cursors::handle_declare_cursor(&query, &mut cursors, &default_client, user_id).await {
                ...
                match enqueue_result {
                    Ok(query_token) => {
                        ...
                        let result = tokio::time::timeout(
                            timeout_duration,
                            execute_query_tls(socket, worker_client, query_router, &query, user_id, smart_scaler)
                        ).await;
            ...
            b'B' => {
                ...
                if result_format_codes.iter().any(|&f| f == 1) {
                    debug!("Bind (TLS): client requested BINARY format for {} columns", 
                    ...
                    if !describe_sent_row_description {
                        ...
                        if let Some(result) = handle_pg_specific_command(&final_sql) {
                            if !result.rows.is_empty() {
                                ...
                                send_command_complete_generic(socket, &cmd_tag).await?;
                                socket.flush().await?;
                                info!("Extended Protocol - Execute: sent intercepted CommandComplete ({})", cmd_tag);
                                prepared_query = None;
                                describe_sent_row_description = false;
                                __describe_column_count = 0;
                                bound_parameters.clear();
                        ...
                        info!("Extended Protocol - Execute: TRUE STREAMING mode, max_rows={}", max_rows);
                        ...
                        match stream_result {
                            Ok(mut stream) => {
                                ...
                                if client_disconnected {
                                    ...
                                    info!("Extended Protocol - Execute: TRUE STREAMING sent {} rows, storing StreamingPortal", rows_sent);
                                    portal_state = Some(PortalState::Streaming(StreamingPortal {
                                        stream, columns, rows_sent, column_count: __describe_column_count, pending_rows,
                                    }));
                                    socket.write_all(&[b's', 0, 0, 0, 4]).await?;
                                    socket.flush().await?;
                                    continue;
